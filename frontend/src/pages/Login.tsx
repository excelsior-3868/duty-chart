...
const Login = () => {
    ...
    // ---------------------------------------------------------------------------
    // SSO — plain redirect (no keycloak-js, no Web Crypto needed)
    // ---------------------------------------------------------------------------
    const SSO_REDIRECT_URI = "http://172.16.49.132:8083/";
    const SSO_BASE_AUTH_URL =
        `http://10.26.192.122:8080/realms/Central-SSWAuth/protocol/openid-connect/auth` +
        `?client_id=duty-chart` +
        `&redirect_uri=${encodeURIComponent(SSO_REDIRECT_URI)}` +
        `&response_type=code` +
        `&scope=openid`;

    // Build the SSO redirect URL — only add prompt=login on retry attempts
    const buildSSOUrl = () => {
        const isRetry = sessionStorage.getItem("sso_attempted") === "true";
        return isRetry ? SSO_BASE_AUTH_URL + "&prompt=login" : SSO_BASE_AUTH_URL;
    };
    ...
    useEffect(() => {
        ...
        // Handle Keycloak error redirect (e.g. wrong password cancelled, access denied)
        const error = params.get("error");
        if (error) {
            // Clean the URL immediately
            window.history.replaceState({}, document.title, window.location.pathname);
            // Mark that SSO has been attempted — next click will use prompt=login
            sessionStorage.setItem("sso_attempted", "true");
            const errorDesc = params.get("error_description") || error;
            toast.error(`SSO error: ${decodeURIComponent(errorDesc.replace(/\+/g, " "))}. Please try again.`);
            return;
        }

        // If Keycloak redirected back with ?code=, exchange it server-side.
        ...
        publicApi
            .post("/v1/auth/sso/keycloak/code/", {
                code,
                redirect_uri: SSO_REDIRECT_URI,
            })
            .then(async (res) => {
                const { access, refresh, id_token } = res.data;
                localStorage.setItem("access", access);
                localStorage.setItem("refresh", refresh);
                localStorage.setItem("login_method", "sso");
                ...
                await refreshUser();
                toast.success("Logged in via NT Single Sign-On");
                navigate(ROUTES.DASHBOARD);
            })
            .catch((err: any) => {
                const msg =
                    err?.response?.data?.detail ||
                    "SSO login failed. Please try again or use manual login.";
                // Mark SSO as attempted so next click uses prompt=login
                sessionStorage.setItem("sso_attempted", "true");
                toast.error(msg);
            })
                ...
                >
                    <CardContent className="p-6">
                        ...
                        <form onSubmit={handleSubmit} className="space-y-3">
                            ...
                            <button
                                type="submit"
                                className="w-full text-white font-medium py-2.5 transition-colors flex items-center justify-center gap-2 bg-primary hover:bg-primary-hover rounded-lg"
                                disabled={submitting}
                            >
                            ...
                            <button
                                ...
                                type="button"
                                onClick={() => {
                                    // Mark as attempted BEFORE redirecting so next retry uses prompt=login
                                    const isRetry = sessionStorage.getItem("sso_attempted") === "true";
                                    sessionStorage.setItem("sso_attempted", "true");
                                    window.location.href = isRetry
                                        ? SSO_BASE_AUTH_URL + "&prompt=login"
                                        : SSO_BASE_AUTH_URL;
                                }}
                                className="w-full flex items-center justify-center gap-2 py-2.5 rounded-lg border-2 border-primary text-primary font-medium hover:bg-primary/5 transition-colors"
                            >
                                <ShieldCheck className="h-4 w-4" />
                                Login via NT Single Sign-On
                            </button>
