...
import {
    ...
    CCOffice,
    CCOfficeResponse,
} from "@/services/ccOffices";
import {
    getAccountingOffices,
    AccountingOffice,
} from "@/services/accountingOffices";
import {
    Table,
    TableBody,
    TableCell,
...
export default function CCOfficesPage() {
    ...
    const [formData, setFormData] = useState({
        name: "",
        accounting_office: null as number | null,
    });

    const { data, isLoading, isError } = useQuery({
        queryKey: ["cc-offices", page, debouncedSearch],
        queryFn: () => getCCOffices({ page, search: debouncedSearch }) as Promise<CCOfficeResponse>,
    });

    const { data: accountingOffices } = useQuery({
        queryKey: ["accounting-offices", "all"],
        queryFn: () => getAccountingOffices({ all: true }) as Promise<AccountingOffice[]>,
    });

    const offices = data?.results || [];

    const createMutation = useMutation({
        mutationFn: createCCOffice,
        onSuccess: () => {
            queryClient.invalidateQueries({ queryKey: ["cc-offices"] });
            setIsAddModalOpen(false);
            resetForm();
            toast.success("CC Office created successfully");
        },
        onError: () => toast.error("Failed to create CC office"),
    });

    const updateMutation = useMutation({
        mutationFn: ({ id, data }: { id: number; data: any }) =>
            updateCCOffice(id, data),
        onSuccess: () => {
            queryClient.invalidateQueries({ queryKey: ["cc-offices"] });
            setIsEditModalOpen(false);
            setSelectedOffice(null);
            resetForm();
            toast.success("CC Office updated successfully");
        },
        onError: () => toast.error("Failed to update CC office"),
    });

    const deleteMutation = useMutation({
        mutationFn: deleteCCOffice,
        onSuccess: () => {
            queryClient.invalidateQueries({ queryKey: ["cc-offices"] });
            setIsDeleteModalOpen(false);
            setSelectedOffice(null);
            toast.success("CC Office deleted successfully");
        },
        onError: () => toast.error("Failed to delete CC office"),
    });
    ...
    const handleEdit = (e: React.FormEvent) => {
        e.preventDefault();
        if (!selectedOffice || !formData.name.trim()) return;
    ...
    const handleDelete = () => {
        ...
        deleteMutation.mutate(selectedOffice.id);
    };

    const openEdit = (office: CCOffice) => {
        setSelectedOffice(office);
        setFormData({
            name: office.name,
            accounting_office: office.accounting_office,
        });
        setIsEditModalOpen(true);
    };

    const openDelete = (office: CCOffice) => {
        setSelectedOffice(office);
    ...
    return (
        <div className="p-6 space-y-6">
                ...
                {isLoading ? (
                    <div className="flex justify-center p-24 bg-white rounded-xl border border-slate-100 shadow-sm">
                        <div className="flex flex-col items-center gap-3">
                            ...
                            <p className="text-xs font-medium text-slate-400">Loading offices...</p>
                        </div>
                    </div>
                ) : isError ? (
                    <div className="flex flex-col items-center justify-center p-12 bg-red-50 rounded-xl border border-red-100 text-center gap-3">
                        ...
                        <div>
                            <p className="text-red-800 font-bold">Failed to load offices</p>
                            <p className="text-red-600/70 text-sm">Please check your connection or permissions.</p>
                        </div>
                        <Button variant="outline" size="sm" onClick={() => queryClient.invalidateQueries({ queryKey: ["cc-offices"] })} className="mt-2 border-red-200 text-red-700 hover:bg-red-100">
                            Try Again
                        </Button>
                    </div>
                ) : (
                                ...
                                <Table>
                                    <TableHeader className="bg-primary hover:bg-primary">
                                        <TableRow className="hover:bg-transparent border-none">
                                            ...
                                            <TableHead className="py-3 text-white font-bold text-sm">Accounting Office</TableHead>
                                            ...
                                            <TableRow>
                                                <TableCell colSpan={4} className="text-center h-48 text-slate-400">
                                                    <div className="flex flex-col items-center gap-2">
                                                        ...
                                                        <p className="font-medium italic">No matching offices found.</p>
                                                    </div>
                                                </TableCell>
                                            </TableRow>
                                        )}
            ...
            <Dialog open={isAddModalOpen} onOpenChange={setIsAddModalOpen}>
                <DialogContent className="sm:max-w-[425px] border-none shadow-2xl overflow-hidden rounded-2xl p-0">
                    <div className="p-6 border-b border-slate-100">
                        <DialogTitle className="text-xl font-bold text-slate-900">New CC Office</DialogTitle>
                        <p className="text-slate-500 text-xs mt-1">Create a new collection centre office.</p>
                    </div>
                    <form onSubmit={handleAdd} className="p-6 space-y-4">
                        <div className="space-y-2">
                            ...
                            <Input
                                id="name"
                                placeholder="e.g. Collection Centre Y"
                                value={formData.name}
                        ...
                        <div className="space-y-2">
                            <Label className="text-xs font-bold text-slate-500">Accounting Office</Label>
                            <Select
                                value={formData.accounting_office ? String(formData.accounting_office) : "none"}
                                onValueChange={(val) => setFormData({ ...formData, accounting_office: val === "none" ? null : Number(val) })}
                            >
            ...
            <Dialog open={isEditModalOpen} onOpenChange={setIsEditModalOpen}>
                <DialogContent className="sm:max-w-[425px] border-none shadow-2xl overflow-hidden rounded-2xl p-0">
                    <div className="p-6 border-b border-slate-100">
                        <DialogTitle className="text-xl font-bold text-slate-900">Edit CC Office</DialogTitle>
                        <p className="text-slate-500 text-xs mt-1">Modify the details of the collection centre office.</p>
                    </div>
                ...
                <DialogContent className="sm:max-w-[400px] border-none shadow-2xl overflow-hidden rounded-2xl p-0">
                    ...
                    <div className="p-6">
                        <p className="text-slate-600 text-sm leading-relaxed">
                            Are you sure you want to delete the office <span className="font-bold text-slate-900">"{selectedOffice?.name}"</span>?
                        </p>
