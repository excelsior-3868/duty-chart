#!/usr/bin/env bash
#
# build-push-arm64.sh — Build, tag, and push dutychart backend and frontend images (ARM64 only) to Nexus
#
# Usage Examples:
#   Interactive release mode (prompts for release yes/no and major/minor bump):
#     ./build-push-arm64.sh
#   Direct version mode (no prompt):
#     ./build-push-arm64.sh v1.0.0
#
...
for COMPONENT in backend frontend; do
  ...
  echo "============================================="
  echo " Building & Publishing $COMPONENT image (ARM64)"
  ...
  if [[ "$COMPONENT" == "frontend" ]]; then
    docker buildx build \
      --platform "$PLATFORMS" \
      -t "$IMAGE_PATH:$VERSION-arm64" \
      -t "$IMAGE_PATH:$IMMUTABLE_TAG-arm64" \
  ...
  echo "============================================="
  echo " Successfully pushed $IMAGE_PATH:$VERSION-arm64"
  echo "============================================="
done
